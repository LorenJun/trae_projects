...
version: "1.1.0"
purpose: "负责从权威实时源采集足球比赛所需的结构化数据，并为预测流程提供可直接使用的快照输入"
---
...
3. 优先使用当前项目已有脚本：okooo_fetch_daily_schedule.py、okooo_save_snapshot.py、prediction_system.py collect-data。
4. 大小球真实数据优先从 handicap.php 页面内的“大小球”tab 获取，不默认使用固定 2.5。
5. 若球队在赛程中显示简称，必须结合 okooo_team_aliases.json 做匹配。
6. 若实时源异常或依赖缺失，允许降级，但必须明确标记为 mock/降级数据。
7. 正式主流程输出以 teams_2025-26.md 为准，不要写入旧 predictions/ 或 reports/ 主流程文件。
8. 可以输出结构化快照、伤停、阵容、赛程信息，但不要替代比赛分析Agent或赔率分析Agent输出终版推荐。
```
...
项目支持在预测阶段（不是采集阶段）通过 Sofascore 自动注入双方近 N 场的状态信息（阵型/控球/上一场首发/球员评分趋势），用于提升“基本面维度”的结构化输入。
...
1. 确认目标比赛（league + date + home_team + away_team + 可选 time）
   ↓
2. 如未知 match_id，先抓当天联赛赛程
   ↓
3. 从赛程 JSON 中定位 match_id
   ↓
4. 若存在简称差异，结合 okooo_team_aliases.json 修正
   ↓
5. 调用 okooo_save_snapshot.py 抓实时快照
   ↓
6. 检查快照内是否包含：欧赔 / 亚值 / 大小球 / 凯利
   ↓
7. 输出结构化数据，交给预测流程使用
```
