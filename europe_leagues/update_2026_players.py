#!/usr/bin/env python3
"""模块说明：批量更新 2026 赛季球员数据与相关资料。

更新球员JSON文件为2026赛季最新数据"""

import os
import json
from datetime import datetime
import logging

# 配置日志
logging.basicConfig(
    level=logging.INFO,
    format='%(asctime)s - %(levelname)s - %(message)s',
    filename='update_2026_players.log'
)

# 定义联赛目录
LEAGUES = [
    'premier_league',
    'la_liga',
    'serie_a',
    'bundesliga',
    'ligue_1'
]

# 项目根目录
ROOT_DIR = '/Users/lin/trae_projects/europe_leagues'

def update_player_file(file_path):
    """更新单个球员文件为2026赛季数据"""
    try:
        with open(file_path, 'r', encoding='utf-8') as f:
            data = json.load(f)
        
        # 更新为2026赛季数据
        current_time = datetime.now().isoformat()
        data['last_updated'] = current_time
        data['season'] = '2026'
        
        # 确保球员数据结构完整
        if 'players' not in data:
            data['players'] = []
        
        # 更新每个球员的最后更新时间
        for player in data.get('players', []):
            player['last_updated'] = current_time
            # 确保球员数据字段完整
            required_fields = ['name', 'position', 'age', 'nationality', 'transfer_status', 'join_date', 'contract_until']
            for field in required_fields:
                if field not in player:
                    player[field] = '' if field != 'age' else 0
            
            # 添加2026赛季统计数据
            if 'stats' not in player:
                player['stats'] = {
                    'appearances': 0,
                    'goals': 0,
                    'assists': 0,
                    'yellow_cards': 0,
                    'red_cards': 0
                }
        
        # 保存更新后的数据
        with open(file_path, 'w', encoding='utf-8') as f:
            json.dump(data, f, ensure_ascii=False, indent=2)
        
        logging.info(f"更新了文件: {file_path}")
        return True
    except Exception as e:
        logging.error(f"更新文件失败 {file_path}: {e}")
        return False

def main():
    """主函数"""
    print("=" * 60)
    print("更新球员JSON文件为2026赛季最新数据")
    print("=" * 60)
    
    total_updated = 0
    total_files = 0
    
    for league in LEAGUES:
        league_dir = os.path.join(ROOT_DIR, league, 'players')
        if not os.path.exists(league_dir):
            logging.warning(f"联赛目录不存在: {league_dir}")
            continue
        
        print(f"\n处理联赛: {league}")
        
        # 获取所有JSON文件
        json_files = [f for f in os.listdir(league_dir) if f.endswith('.json')]
        league_files = len(json_files)
        league_updated = 0
        
        for json_file in json_files:
            file_path = os.path.join(league_dir, json_file)
            total_files += 1
            if update_player_file(file_path):
                league_updated += 1
                total_updated += 1
        
        print(f"  处理文件: {league_files}, 更新成功: {league_updated}")
    
    print("\n" + "=" * 60)
    print(f"更新完成！")
    print(f"总计处理: {total_files} 个文件")
    print(f"更新成功: {total_updated} 个文件")
    print("=" * 60)

if __name__ == "__main__":
    main()