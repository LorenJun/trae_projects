...
class EnhancedPredictor:
    ...
    def _detect_market_odds_anomaly(
        ...
        # Cross-check with asian handicap direction/strength.
        if isinstance(asian_handicap, dict):
        ...
        # Cross-check against base model prior.
        if base_home_lambda and base_away_lambda:
            ...
            base_probs = dc.predict_with_dixon_coles(
                max(0.15, float(base_home_lambda)),
                max(0.15, float(base_away_lambda)),
            )
            ...
            diag["base_model_probs"] = {
                "home": round(base_home, 4),
                "draw": round(float(base_probs.get("draw") or 0.0), 4),
    ...
    def _calibrate_lambdas_from_market(
        ...
        anomaly_diag = self._detect_market_odds_anomaly(
            league_code=league_code,
            european_odds=european_odds,
            asian_handicap=asian_handicap,
        ...
        if anomaly_diag.get("level") == "high":
            diag.update(
                {
                    "applied": False,
                    "reason": "market_odds_anomaly_high",
                    "kept_base_lambda": {
                        "home": round(float(base_home_lambda), 3),
                        "away": round(float(base_away_lambda), 3),
                        "total": round(float(base_home_lambda + base_away_lambda), 3),
                    },
                }
            )
            return base_home_lambda, base_away_lambda, diag
    ...
    def _load_league_table(self, league_code: str) -> Dict[str, Dict[str, Any]]:
        ...
        if cached:
            return cached
        path = os.path.join(self.base_dir, league_code, "teams_2025-26.md")
    ...
    def predict_match(
        ...
        # 1.1 构建多维比赛画像：球队基本面/战术/控球/伤停/H2H/盘口/资金流向代理
        match_intelligence = self._build_match_intelligence(
            league_code=league_code,
            home_team=home_team,
            away_team=away_team,
            match_date=match_date,
        ...
        try:
            mot = match_intelligence.get("motivation") if isinstance(match_intelligence, dict) else {}
            if "home_motivation" not in analysis_context and isinstance(mot, dict):
                sugg = mot.get("suggested_scores") or {}
                if sugg.get("home") is not None:
                    analysis_context["home_motivation"] = float(sugg["home"])
                if sugg.get("away") is not None:
                    analysis_context["away_motivation"] = float(sugg["away"])
            h2h_ctx = match_intelligence.get("head_to_head") if isinstance(match_intelligence, dict) else {}
            if isinstance(h2h_ctx, dict) and h2h_ctx.get("available"):
                analysis_context.setdefault("h2h_home_wins", int(h2h_ctx.get("home_wins", 0)))
                analysis_context.setdefault("h2h_away_wins", int(h2h_ctx.get("away_wins", 0)))
                analysis_context.setdefault("h2h_draws", int(h2h_ctx.get("draws", 0)))
        except Exception as e:
            realtime["context_applied"]["match_intelligence_bind_error"] = str(e)
        ...
        # 使用多维画像做轻量 λ 修正，让控球/阵型/近况/伤停/资金流向代理真正参与结果。
        try:
            qa = match_intelligence.get("quant_adjustment") if isinstance(match_intelligence, dict) else {}
            if isinstance(qa, dict):
                hs = float(qa.get("home_lambda_scale") or 1.0)
                as_ = float(qa.get("away_lambda_scale") or 1.0)
                home_lambda = max(0.15, home_lambda * hs)
                away_lambda = max(0.15, away_lambda * as_)
                realtime["context_applied"]["match_intelligence_lambda_scale"] = {
                    "home_lambda_scale": hs,
                    "away_lambda_scale": as_,
                }
        except Exception as e:
            realtime["context_applied"]["match_intelligence_lambda_scale"] = {"error": str(e)}
        ...
        # 再叠加一次多维画像的轻量修正，让战术/控球/伤停/资金流向代理体现在最终概率上。
        final_prob, match_intel_diag = self._apply_match_intelligence_adjustment(
            final_prob=final_prob,
        ...
        match_intelligence = self._finalize_match_intelligence(
            match_intelligence=match_intelligence,
            historical_odds_reference=historical_odds_reference,
        ...
        if isinstance(over_under, dict):
            over_under, ou_resonance_diag = self._apply_market_resonance_to_over_under(
            ...
            try:
                ...
                if isinstance(current_odds, dict):
                    ...
                    if isinstance(blk, dict):
                        ...
                        if isinstance(blk.get("consensus"), dict):
                            market_ou["consensus"] = blk.get("consensus") or {}
                        if isinstance(blk.get("companies"), list):
                            market_ou["companies"] = blk.get("companies") or []
                        if blk.get("company_mode"):
                            market_ou["company_mode"] = blk.get("company_mode")
                if market_ou:
