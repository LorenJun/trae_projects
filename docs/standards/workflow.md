...
- 正式 CLI 通过 `prediction_system.py` 发现入口进入，真实命令实现以 `europe_leagues/app/cli.py` 为准
- 历史 RAG 记忆回填使用 `prediction_system.py sync-memory-rag`
- 赛后结果闭环优先使用 `save-result`、`auto-sync-results`、`result-sync-daemon`、`sync-pending-results-review`
...
| 准确率统计 | `python3 prediction_system.py accuracy --refresh --json`（显式重建） | 是 |
