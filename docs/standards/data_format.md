...
> 3. `prediction_system.py predict-match / predict-schedule` 执行增强预测，并自动接入 RAG 记忆层  
...
- 当前正式链路默认只推荐 `handicap.php` 页内 `大小球` tab 作为主来源
- `overunder.php`、`daxiao.php`、桌面 `/ou/` 仅用于兼容或补救，不应在主流程说明中当作默认推荐路径
