...
本 Skill 是当前项目的足球预测主技能。执行时必须遵循“赛程/MatchID -> 实时快照 -> `app/cli.py` -> `DomainPredictor` -> `EnhancedPredictor` -> 写回/统计”的正式链路，而不是只靠静态基本面或固定 `2.5` 大小球线下结论。
...
- `风险`: 冷门 / 平局风险 / 伤停风险
- `结论`: 稳健玩法和进取玩法
- `诊断`: 必要时补充 `realtime.okooo`、`context_applied`、`line_source`、`runtime_profile`
