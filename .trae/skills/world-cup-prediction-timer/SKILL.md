...
4. **完赛自动回填 + 刷新**：每轮对「已开赛但未回填」的比赛调用 `auto_sync_results()`（正式结果闭环 `prediction_system.py auto-sync-results --limit 10 --json`）尝试回填赛果；若有比赛被新回填，则把对应日期并入本轮刷新集合，自动把「终场比分 + 最新大小球/欧赔/亚盘水位」刷上网页，无需人工。
