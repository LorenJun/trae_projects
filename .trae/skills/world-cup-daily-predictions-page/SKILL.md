...
- 胜平负方向 + 信心、三色概率条
- 最可能总进球分布（**只列与大小球判定同侧的总进球档**：判小球→只列 <线 的档，判大球→只列 >线，档内重新归一化后取 Top3，避免「3 球 vs 小球」众数/累积口径打架）、Top 3 比分
- 大小球方向 + 盘口线 + 盘口漂移
- **真实盘口原始数据区块**：欧赔（开→终 + 来源如「13家共识」）、亚盘（开→终 + 水位 + 单家/均盘兜底）、
  凯利（主/平/客）、大小（大/小水位 @ 线）——用于直观证明使用真实数据
- 三轴综合研判（背离/共振/部分一致）+ 让球口诀（`direction_handicap`，如「升盘配高水+欧赔主升·诱上」）+ 临场资金质检（drift_confidence：high 共振 / low 孤证 / n/a）
- 爆冷预警（仅中/高时显示，🟡/🔴）

### Top 比分筛选规则（`_scores_for_side`）

- **方向 = 硬约束**：比分只在「方向允许集」内取，**绝不出反向爆冷比分**（判主胜剔除客胜比分，判客胜剔除主胜比分）。
- **大小球 = 同侧约束**：判大球只出大于盘口线的比分，判小球只出小于的。
- **爆冷/平局放行**：当模型提示爆冷（中/高）或平局概率达标（≥30%）时，按 `_allowed_outcomes` 放行对应的平局/反向胜负比分。
- **凑足 3 个**：严格双约束下不足 3 个时，看胜平负**第二高方向**，从该方向补概率最大且未重复的比分凑足；仍为空才回退到「胜平负概率最高一侧」的比分（`_fallback_top_outcome`，仍不放开方向）。
- **概率口径**：在候选允许集内重新归一化，chip 显示的是「该研判约束下」的条件概率。
