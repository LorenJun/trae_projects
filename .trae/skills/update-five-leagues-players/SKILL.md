...
description: "更新五大联赛 players/*.json（名单→中文名/号码→统计/热区→审计），并与整改后的 `domain/team_strength.py`、联赛目录和主预测链保持一致。当需要批量刷新五大联赛球员数据或赛季更新时调用。"
---
...
整改后口径下，它属于“联赛数据资产维护”能力，不直接改预测主链代码，但会为以下模块提供输入：
...
- 数据归属：各联赛 `players/*.json` 是球队实力分析和球员上下文的静态数据层
- 下游消费方：`domain/team_strength.py`、预测主链中的球队实力分析逻辑
- 不负责内容：不修改 `app/cli.py`、`prediction_system.py`、`teams_2025-26.md`、`prediction_archive.json`
- 正确边界：这个 Skill 维护的是球员数据资产，不是赛程 SoT 或预测结果写回
...
说明：
- 历史文档里提到的 `update_schedules_from_500.py` 当前仓库内不存在
- 本 Skill 关注 players 数据，赛程更新不作为前置条件
